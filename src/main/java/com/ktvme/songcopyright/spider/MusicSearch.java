...
public class MusicSearch {
    ...
    public static List<SongCopyrightDO> searchCopyrightFromWYY(SongDO songDO) {
        ...
        JsonObject songObjects = CopyrightSearchUtil.searchCopyrightFromWYY(title);
        if (songObjects == null) {
    ...
    public static List<SongCopyrightDO> searchCopyrightFromQQ(SongDO songDO, Integer current, Integer size) {
        String title = songDO.getSongTitle();
        JsonObject songObjects = CopyrightSearchUtil.searchMusicInfoFromQQ(title, current.toString(), size.toString());
        if (songObjects == null) {
            log.info("[INF] ---> QQ音乐没有 {} 这首歌", title);
            return null;
        }

        JsonArray songList = songObjects.getAsJsonObject("data")
